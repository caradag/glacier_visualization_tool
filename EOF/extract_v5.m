function [p_out, t_out] = extract_v5(tstart, tfinal, t_in, p_in, dt,...
    max_nan, max_succ_nan, interp_meth, max_dt, max_miss, max_miss_int,...
    max_miss_out)
...
% values.
% v5 also outputs p_out as NaN's if tolerances for missing data points 
% within the interval or at its ends are exceeded, these tolerances are
% max_miss, max_miss_int and max_miss_out as described below. 
...
if nargin < 9 || isempty(max_dt)
    max_dt =  20/60/24;
end

if nargin < 10 || isempty(max_miss);
    max_miss = max_succ_nan;
end

if nargin < 11 || isempty(max_miss_int);
    max_miss_int = max_succ_nan;
end

if nargin < 12 || isempty(max_miss_out);
