function output = sensor_read_2014_v2(type,sensor_list,start,finish,raw,tseries_break,use_offset_data,CR10,use_textscan)
...
for ii=1:n_sensor
    ...
    [log_read,log_type,chan_read,date_start,date_finish,sensor_ID,grid,position,atpress,atpress_CR1K,multiplier,icepress]  = identify(type,sensor_list{ii},inst,start, finish);
    ...
    log_read = log_read(date_order); log_type = log_type(date_order); chan_read = chan_read(date_order);
...
function [log_read,log_type,chan_read,date_start,date_finish,sensor_ID,grid,position,atpress,atpress_CR1K,multiplier,icepress]  = identify(type,sensor,inst,start,finish)
...
for ii=1:n_entries
    if ((strcmp(type,'sensor') && strcmpi(sensor,inst.sensor_ID{ii})) ||  (strcmp(type,'grid') && strcmpi(sensor,inst.sensor_grid{ii}))...
        ...
        disp(['  sensorRead -> Logger type: ' inst.logger_type{ii}]);
...
disp(['    loggerRead -> log_read: ' log_read ', log_type: ' log_type ', chan_read: ' num2str(chan_read) ...
      ', start_year/day: ' num2str(start_year) '/' num2str(start_day)...
      ', finish_year/day: ' num2str(finish_year) '/' num2str(finish_day)]);
...
for year_nominal=start_year:finish_year+1
    for ii = 1:2
        ...
        for suffix=0:filespec.suff_max;
            ...
            elseif suffix < 1000
                filename_read=strcat(filespec.path,'/',num2str(year_nominal),'/',season,'/loggers/raw','/',log_read,'.',num2str(suffix));                
                if exist(filename_read,'file'), warning('suffix > 100, no par file'), end
            end
            if exist(filename_offset,'file') && use_offset_data
                [~, offset_read] = textread(filename_offset,'%s %s',1,'delimiter','=');
                ...
                if ver > 6 && use_textscan
                    ...
                    elseif strcmp(log_type,'CR1000')
                        ...
                        for ii=1:n_data_in
                            ...
                            if strcmp(press_str{ii},'"NAN"') || strcmp(press_str{ii},'')
                                press(ii)=NaN;
                            else
                    ...
                    elseif strcmp(log_type,'CR1000')
                        ...
                        for ii=1:n_data_in
                            ...
                            if ii > length(press_str) || strcmp(press_str{ii},'"NAN"') || strcmp(press_str{ii},'')
                                press(ii)=NaN;
                            else
                            press(ii)=str2num(press_str{ii});
                            end
            ...
            clear year day hours minutes battvolt temp press
        end
    end
end
