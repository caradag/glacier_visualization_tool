...
filename_info = '../../DATA/Pressure_Sensor_Reference_Table_2011.csv';       %comma-delimited file containing logger / sensor / hole information
...
[inst.logger_ID,inst.logger_type,inst.channel,inst.sensor_ID,inst.hole_ID,inst.thickness,...
    ...
    = textread(filename_info,'%s %s %f %s %s %f %s %f %f %f %f %f %f %f %f %f %f %f %f %f','delimiter',',',...
...
for ii=1:n_sensor
    sensor_list{ii};
    [log_read,log_type,chan_read,date_start,date_finish,sensor_ID,grid,position,atpress,icepress]  = identify(type,sensor_list{ii},inst,start, finish);
    ...
    if ~raw
        %Convert Raw Data into Effective Pressure
...
function [log_read,log_type,chan_read,date_start,date_finish,sensor_ID,grid,position,atpress,icepress]  = identify(type,sensor,inst,start,finish)
        ...
        for suffix=0:filespec.suff_max;
            ...
            if exist(filename_read,'file')
                ...
                if ver > 6 && use_textscan
                    disp(strcat('really:',num2str(ver(1)),' > 6     FILE: ',filename_read))
                    ...
                    elseif strcmp(log_type,'CR1000')
                        ...
                        data_in=textscan(fid, '%s %u %s %s %s %s %s %s %s %s', 'delimiter', ',','emptyvalue',NaN,'headerlines',4);
